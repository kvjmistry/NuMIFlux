...
//___________________________________________________________________________
int NuMIFlux::calcEnuWgt( bsim::Dk2Nu* decay, const TVector3& xyz,
												 double& enu, double& wgt_xy)
{
		...
		if ( parentp > 0. ) {
				costh_pardet = ( decay->decay.pdpx*(xpos-decay->decay.vx) +
												...
												decay->decay.pdpz*(zpos-decay->decay.vz) )
				/ ( parentp * rad);
		...
		if ( decay->decay.ptype == kpdg_muplus || decay->decay.ptype == kpdg_muminus) {
				...
				switch ( decay->decay.ntype ) {
						...
						case kpdg_numubar:
						{
								double xnu = 2.0 * enuzr / kMUMASS;
								wgt_ratio = ( (3.0-2.0*xnu )  - (1.0-2.0*xnu)*costh ) / (3.0-2.0*xnu);
								if (xnu > 1) wgt_ratio = 0; // Set to zero for now, can set to 1 in the future and treat captures as isotropic
								break;
						}
						default:
