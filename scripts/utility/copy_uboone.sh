...
for i in $(ifdh ls /pnfs/numix/flux/ME_FHC_G4NuMI_v6r1_0th/me000z200i/g4numi);do
	#echo "$i"
	if [ $i == "" -o  $i == "/pnfs/numix/flux/ME_FHC_G4NuMI_v6r1_0th/me000z200i/g4numi/" ]; then echo "skipping copy of $i";continue; fi
	...
	if [ !  -e $(echo "$i" | cut -d'/' -f8) ]; then echo " copying file $(echo "$i" | cut -d'/' -f8)";ifdh cp $i ./; fi
done
