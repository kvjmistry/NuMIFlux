...
// ----------------------------------------------------------------------------
// Main
void plot_parent_flux_angle(const char* horn, TString mode) { // (fhc/rhc, numu/nue)
	...
	double rebin{2}; // number to rebin the histograms by
	...
	switch (return_mode(mode)){
		case enumu:
			std::cout << "\nUsing Numu Mode!\n" << std::endl;
			Gethist_CV_AV = "numu/Detsmear/Th_numu_CV_TPC";      // CV Flux using the 5MeV bins for now 
			mode_char = "numu";
			break;

		case enue:
			std::cout << "\nUsing Nue Mode!\n" << std::endl;
			Gethist_CV_AV = "nue/Detsmear/Th_nue_CV_TPC";
			mode_char = "nue";
			break;

		case enumubar:
			std::cout << "\nUsing Numubar Mode!\n" << std::endl;
			Gethist_CV_AV = "numubar/Detsmear/Th_numubar_CV_TPC";
			mode_char = "numubar";
			break;

		case enuebar:
			std::cout << "\nUsing Nuebar Mode!\n" << std::endl;
			Gethist_CV_AV = "nuebar/Detsmear/Th_nuebar_CV_TPC";
			mode_char = "nuebar";
			break;
	...
	// FHC File in 
	if (!strcmp(horn, "fhc")) {
		boolfile  = GetFile(f,"/uboone/data/users/kmistry/work/PPFX/uboone/beamline_zero_threshold_v46/FHC/sept/output_uboone_fhc_run0_set1.root");
		if (boolfile == false) gSystem->Exit(0);
	}
	else { // RHC file
		boolfile  = GetFile(f,"/uboone/data/users/kmistry/work/PPFX/uboone/beamline_zero_threshold_v46/RHC/sept/output_uboone_rhc_run0_set1.root");
	...
	// Define Integrals
  double energy_threshold = 150;
  std::cout <<"Using Angle Threshold of: " << energy_threshold << std::endl;
  double xbin_th = h_ppfx_flux->GetXaxis()->FindBin(energy_threshold);   // find the x bin to integrate from
	double flux_cv  = h_ppfx_flux->Integral(0, xbin_th) * normfactor;
	...
	h_ppfx_flux->GetXaxis()->SetRangeUser(6,180.0);
	h_ppfx_flux->SetLineStyle(2);

	if (mode == "numu")		h_ppfx_flux->SetTitle("#nu_{#mu}; Angle [deg]; #nu / POT / 2 deg / cm^{2}");
	if (mode == "nue")		h_ppfx_flux->SetTitle("#nu_{e}; Angle [deg]; #nu / POT / 2 deg / cm^{2}");
	if (mode == "numubar")	h_ppfx_flux->SetTitle("#bar{#nu_{#mu}}; Angle [deg]; #nu / POT / 2 deg / cm^{2}");
	if (mode == "nuebar")	h_ppfx_flux->SetTitle("#bar{#nu_{e}}; Angle [deg]; #nu / POT / 2 deg / cm^{2}");
	...
	for (unsigned int i = 0; i < parent.size(); i++){
		boolhist = GetHist(f, h_parent.at(i), Form("%s/%s/Th_%s_%s_AV_TPC", mode_char, parent.at(i).c_str(),mode_char, parent.at(i).c_str() )); if (boolhist == false) gSystem->Exit(0);
    	xbin_th = h_parent.at(i)->GetXaxis()->FindBin(energy_threshold);   // find the x bin to integrate from
		flux_parent.at(i) =  h_parent.at(i)->Integral(0, xbin_th) * normfactor;
	...
	c1->Print(Form("plots/parent/parentflux_%s_%s_angle.pdf", horn, mode_char));
	std::cout << "\n"<< std::endl;
